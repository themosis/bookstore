...
class PageRenderer
{
	...
	public function settings($args)
	{
		...
		switch ($type) {
            ...
            case 'password':
                ...
                static::displayPassword($args['section'].'['.$name.']', $name, $setting[$name]);
                break;
	...
	*/
	...
	/**
	* Display password inputs
	*
	* @param string name attribute for the field
	* @param string (optional) the saved value
	*
	*/
	private static function displayPassword($name, $id, $value = '')
	{
		?>
			<input type="password" name="<?php echo($name); ?>" data-type="text" class="large-text" id="<?php echo($id); ?>" value="<?php if (isset($value)) echo(esc_attr($value)); ?>" />
		<?php
	}
